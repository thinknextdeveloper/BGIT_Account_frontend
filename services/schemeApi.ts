import { ApiResponse, MasterSchemeRecord } from "@/types/scheme";
...
export async function createScheme(data: { collegeName: string; scheme: string }): Promise<ApiResponse<null>> {
  return fetchWithAuth<ApiResponse<null>>("/scheme/add", {
    method: "POST",
    body: JSON.stringify(data),
  });
}

export async function display(collegeName?: string): Promise<ApiResponse<MasterSchemeRecord[]>> {
  const params = collegeName ? `?collegeName=${encodeURIComponent(collegeName)}` : "";
  return fetchWithAuth<ApiResponse<MasterSchemeRecord[]>>(`/scheme/display${params}`);
}
