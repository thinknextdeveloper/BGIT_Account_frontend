...
import { useMasterScheme } from "@/hooks/useMasterScheme";
import { schemeApi, createScheme } from "@/services/schemeApi";
...
export default function SchemePage() {
  ...
  const [newRowScheme, setNewRowScheme] = useState("");
  ...
  /**
   * Export Scheme table data to Excel (.csv file with UTF-8 BOM)
   */
  const exportToExcel = () => {
    ...
    let csvContent = "CollegeName,Scheme\n";
    filteredRecords.forEach((row) => {
      const college = `"${(row.CollegeName || "").replace(/"/g, '""')}"`;
      const scheme = `"${(row.Scheme || "").replace(/"/g, '""')}"`;
      csvContent += `${college},${scheme}\n`;
    });
    ...
    link.setAttribute("download", `Scheme_Master_Report_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  /**
   * Print Scheme table data / Save as PDF
   */
      ...
      .map(
        (row, idx) => `
          <tr>
            ...
            <td style="padding: 8px; border: 1px solid #cbd5e1;">${row.Scheme || ""}</td>
          </tr>
        `
      )
      .join("");
  ...
  /**
   * Equivalent to legacy btnSaveNewData_Click validation and addRow() execution
   */
  const btnSaveNewData_Click = async () => {
    setValidationError(null);
    setSuccessMessage(null);

    // Legacy Validation: Verify that the last row has all required values filled out
    if (!newRowCollege || !newRowScheme || newRowCollege.trim() === "" || newRowScheme.trim() === "") {
      setValidationError("You have to firstly set all values in last row");
      return;
    }

    try {
      setSaving(true);
      const saveFn = schemeApi?.createScheme || createScheme;
      const res = await saveFn({
        collegeName: newRowCollege.trim(),
        scheme: newRowScheme.trim(),
      });

      setSuccessMessage(res.message || "New record added successfully!");
      setNewRowCollege("");
      setNewRowScheme("");
      refetch(); // Refresh table data without full page reload

      setTimeout(() => {
        setSuccessMessage(null);
      }, 4000);
    } catch (err: any) {
      setValidationError(err.message || "Failed to add new record.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-gray-200 pb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Display and manage schemes from MasterScheme table.
          </p>
        </div>
          ...
          <table className="w-full text-left text-sm text-gray-800">
            <thead className="bg-gray-50 text-xs uppercase tracking-wider text-gray-500 border-b border-gray-200">
              <tr>
                ...
                <th scope="col" className="px-6 py-3.5 font-semibold" style={{ width: "150px" }}>Scheme</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <>
                  {/* MasterScheme Data Rows */}
                  {filteredRecords.map((row, index) => (
                    <tr
                      key={`${row.CollegeName}-${row.Scheme}-${index}`}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-700">{row.Scheme}</td>
                    </tr>
                  ))}

                  {/* Empty Row at the end for new entry */}
                  <tr className="bg-emerald-50/40 border-2 border-dashed border-emerald-300">
                    ...
                    <td className="px-6 py-3.5">
                      <input
                        ...
                        value={newRowScheme}
                        onChange={(e) => setNewRowScheme(e.target.value)}
                        className="w-full bg-white border border-emerald-300 text-gray-800 text-sm rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"
                      />
                    </td>
                  </tr>
