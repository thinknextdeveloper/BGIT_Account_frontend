...
export default function SearchAddressPage() {
  ...
  const handleSearch = async () => {
    ...
    if (searchByAddress.rejected.match(actionResult)) {
          ...
          <table className="w-full text-xs text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-200 text-gray-700">
              {results && results.length > 0 ? (
                results.map((row: AddressSearchRow, idx: number) => (
                  <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="py-2 px-3 border-r border-gray-200">{renderSafeText(row.Class)}</td>
                    <td className="py-2 px-3 border-r border-gray-200">{renderSafeText(row.FatherName)}</td>
                    <td className="py-2 px-3 border-r border-gray-200">{renderSafeText(row.PhoneNo)}</td>
                    <td className="py-2 px-3 border-r border-gray-200">{renderSafeText(row.StudentMobileNo)}</td>
                    <td className="py-2 px-3 border-r border-gray-200">{renderSafeText(row.FatherMobileNo)}</td>
                    <td className="py-2 px-3 border-r border-gray-200">{renderSafeText(row.MotherMobileNo)}</td>
                    <td className="py-2 px-3">{renderSafeText(row.PermanentAddress)}</td>
                  </tr>
                ))
              ) : (
                <tr>
