...
import {
  ...
  searchByClassRollNo,
  clearResults,
  ClassRollNoSearchRow,
} from "@/store/slices/searchByClassRollNoSlice";
...
export default function SearchByClassRollNoPage() {
  const dispatch = useDispatch<AppDispatch>();
  const { colleges: reduxColleges, results, loading } = useSelector(
    (state: RootState) => state.searchByClassRollNo
  );
  ...
  const classRollNoInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleSearch = async () => {
    ...
    if (!classRollNo.trim()) {
      setFormError("Please specify class roll No");
      classRollNoInputRef.current?.focus();
      return;
    }
    ...
    if (isNaN(Number(classRollNo.trim()))) {
      setFormError("Class Roll No must be Numeric");
      classRollNoInputRef.current?.focus();
      return;
    }
    ...
    if (searchByClassRollNo.rejected.match(actionResult)) {
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-5 mb-5 max-w-4xl">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
          <div className="flex-1 space-y-4">
            ...
            {/* Class Roll No Input Row */}
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <label className="text-sm font-semibold text-gray-700 w-44">
                Enter Class Roll No
              </label>
              <input
                ref={classRollNoInputRef}
                type="text"
                value={classRollNo}
                onChange={(e) => {
                  setClassRollNo(e.target.value);
                  setFormError(null);
                }}
          ...
          <table className="w-full text-xs text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-gray-200 text-gray-700">
              {results && results.length > 0 ? (
                results.map((row: ClassRollNoSearchRow, idx: number) => (
                  <tr key={idx} className="hover:bg-blue-50/50 transition-colors">
                    {COLUMN_KEYS.map((key) => (
                      <td
                        key={key}
                        className={`py-2 px-3 border-r border-gray-200 ${
                          key === "ClassRollNo" || key === "IDNo" || key === "StudentName"
