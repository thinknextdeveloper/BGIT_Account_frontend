...
import {
  ...
  searchTransaction,
  clearResults,
} from "@/store/slices/searchTransactionIdSlice";

export default function SearchTransactionIDPage() {
  const dispatch = useDispatch<AppDispatch>();
  const { colleges: reduxColleges, results, loading, error } = useSelector(
    (state: RootState) => state.searchTransactionId
  );
  ...
  const handleFind = async () => {
    ...
    if (searchTransaction.rejected.match(actionResult)) {
        ...
        <div className="bg-gray-200 border border-gray-300 rounded min-h-[420px] max-h-[600px] overflow-auto">
          {results && results.length > 0 ? (
            ...
            <div className="h-[420px] flex items-center justify-center text-gray-500 text-sm">
              {loading ? "Searching..." : "No records to display. Enter a Transaction ID and click Find."}
            </div>
          )}
        </div>
      </div>
