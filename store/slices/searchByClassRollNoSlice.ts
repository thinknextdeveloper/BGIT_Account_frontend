...
export interface ClassRollNoSearchRow {
...
interface SearchByClassRollNoState {
  colleges: string[];
  results: ClassRollNoSearchRow[];
  loading: boolean;
  error: string | null;
}

const initialState: SearchByClassRollNoState = {
  colleges: [],
  results: [],
  loading: false,
  error: null,
...
export const fetchColleges = createAsyncThunk(
  "searchByClassRollNo/fetchColleges",
...
export const searchByClassRollNo = createAsyncThunk(
  "searchByClassRollNo/find",
  async (
    params: {
      classRollNo: string;
      college?: string;
    ...
    const queryParams: Record<string, string> = {
      classRollNo: params.classRollNo,
      allColleges: String(params.allColleges),
    };
    if (!params.allColleges && params.college) {
      queryParams.college = params.college;
    }
    const res = await reduxApiClient.get("search-by-class-roll-no/find", queryParams);
    if (!res.success) return rejectWithValue(res.error?.message ?? "Something went wrong");
    return res.data?.data ?? [];
  }
);

const searchByClassRollNoSlice = createSlice({
  name: "searchByClassRollNo",
  initialState,
  reducers: {
    clearResults(state) {
      state.results = [];
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(searchByClassRollNo.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(searchByClassRollNo.fulfilled, (state, action: any) => {
        state.loading = false;
        state.results = action.payload || [];
      })
      .addCase(searchByClassRollNo.rejected, (state, action: any) => {
        state.loading = false;
...
export const { clearResults } = searchByClassRollNoSlice.actions;
export default searchByClassRollNoSlice.reducer;
