...
interface SearchFacultyIdNoState {
  colleges: string[];
  results: FacultyRow[];
  loading: boolean;
  error: string | null;
...
const initialState: SearchFacultyIdNoState = {
  colleges: [],
  results: [],
  loading: false,
  error: null,
...
// Uses existing master-course/colleges API
export const fetchColleges = createAsyncThunk(
  "searchFacultyIdNo/fetchColleges",
...
export const searchFacultyByIDNo = createAsyncThunk(
  "searchFacultyIdNo/find",
  async (
    params: { idNo: string; college?: string; allColleges: boolean },
    { rejectWithValue }
  ) => {
    ...
    const res = await reduxApiClient.get("search-faculty-id-no/find", queryParams);
    if (!res.success) return rejectWithValue(res.error?.message ?? "Something went wrong");
    return res.data.data;
  }
);

const searchFacultyIdNoSlice = createSlice({
  name: "searchFacultyIdNo",
  initialState,
  reducers: {
    clearResults: (state) => {
      state.results = [];
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(searchFacultyByIDNo.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(searchFacultyByIDNo.fulfilled, (state, action: any) => {
        state.loading = false;
        state.results = Array.isArray(action.payload) ? action.payload : [];
        state.error = null;
      })
      .addCase(searchFacultyByIDNo.rejected, (state, action: any) => {
        state.loading = false;
        state.results = [];
...
export const { clearResults } = searchFacultyIdNoSlice.actions;
export default searchFacultyIdNoSlice.reducer;
